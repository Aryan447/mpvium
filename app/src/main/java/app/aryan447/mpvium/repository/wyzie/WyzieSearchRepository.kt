...
import app.aryan447.mpvium.repository.subtitles.CinemetaMetaResponse
import app.aryan447.mpvium.repository.subtitles.CinemetaResolver
import app.aryan447.mpvium.repository.subtitles.CinemetaSearchResponse
import app.aryan447.mpvium.repository.subtitles.OpenSubtitlesProvider
import app.aryan447.mpvium.repository.subtitles.SubSourceProvider
import app.aryan447.mpvium.repository.subtitles.SubtitleProvider
import app.aryan447.mpvium.repository.subtitles.SubtitleQuery
import app.aryan447.mpvium.repository.subtitles.SubtitleZip
import app.aryan447.mpvium.repository.subtitles.WyzieAggregateProvider
...
class WyzieSearchRepository(
    ...
    private val resolver = CinemetaResolver(client, json)
    private val openSubtitlesProvider = OpenSubtitlesProvider(client, json, resolver)
    private val subSourceProvider = SubSourceProvider(client, json, resolver)
    private val wyzieAggregateProvider = WyzieAggregateProvider(client, json, preferences, resolver)

    /** All subtitle providers, searched concurrently. OpenSubtitles stays first. */
    private val providers: List<SubtitleProvider> = listOf(
        openSubtitlesProvider,
        subSourceProvider,
        wyzieAggregateProvider
    )
    ...
    companion object {
        ...
        /** Per-provider ceiling so one slow source never stalls the merged list. */
        ...
        try {
            ...
            val allowedLanguages =
                if (selectedLangsRaw.isNotEmpty() && !selectedLangsRaw.contains("all")) {
                    selectedLangsRaw.map { it.lowercase() }.toSet()
                } else null
            val subtitleQuery = SubtitleQuery(
                query = query,
                season = season,
                episode = episode,
                year = year,
            ...
            // Search every available provider concurrently. A provider that
            // fails or times out only loses its own results.
            val outcomes = supervisorScope {
                providers.filter { it.isAvailable }.map { provider ->
                    async {
                        val result = withTimeoutOrNull(PROVIDER_TIMEOUT_MS) {
                            runCatching { provider.search(subtitleQuery) }
                        } ?: Result.failure(IOException("${provider.displayName} timed out"))
                        result.onFailure {
                            Log.w("WyzieSearchRepository", "${provider.displayName} search failed: ${it.message}")
                        }
                        result
                    }
                }.map { it.await() }
            ...
            if (outcomes.isEmpty()) {
                return@withContext Result.failure(IOException("No subtitle providers available."))
            }
            // Same file from two providers (identical URL) is listed once.
            // Results that merely look alike are kept separately with their
            // source badge so no legitimate release is ever dropped.
            val merged = outcomes
                .flatMap { result -> result.getOrDefault(emptyList()) }
                .distinctBy { it.url }
            val failures = outcomes.mapNotNull { it.exceptionOrNull() }
            if (merged.isEmpty() && failures.isNotEmpty()) {
                return@withContext Result.failure(failures.first())
            }
            Result.success(filterAndSort(merged, query))
        } catch (e: Exception) {
    ...
    suspend fun download(subtitle: WyzieSubtitle, mediaTitle: String): Result<Uri> = withContext(Dispatchers.IO) {
        try {
            val raw: Pair<ByteArray, String?> = if (subtitle.url.startsWith("subsource://")) {
                subSourceProvider.downloadBytes(subtitle)
            } else {
                ...
                val payload = response.body?.bytes() ?: return@withContext Result.failure(Exception("Empty body"))
                payload to null
            }

            val urlExtension = subtitle.url.substringAfterLast("/", "")
                .substringBefore("?")
                .substringAfterLast(".", "")
                .takeIf { it.matches(Regex("[A-Za-z0-9]{2,5}")) }
            val fallbackExtension = subtitle.format?.lowercase()?.takeIf { it.isNotBlank() }
                ?: raw.second
                ?: urlExtension
                ?: "srt"

            // Providers such as SubSource return a ZIP archive; extract the
            // playable subtitle file. Plain payloads pass through untouched.
            val (bytes, extension) = SubtitleZip.unwrapIfZip(raw.first, fallbackExtension)
    ...
    suspend fun searchMedia(query: String): Result<List<WyzieTmdbResult>> = withContext(Dispatchers.IO) {
        try {
            ...
            val tmdbResults = runCatching {
                resolver.tmdbSearch(expandedQuery, preferences.wyzieApiKey.get().trim().takeIf { it.isNotBlank() })
            }.getOrNull()
            ...
            if (expandedQuery != query) {
                val originalResults = runCatching {
                    resolver.tmdbSearch(query, preferences.wyzieApiKey.get().trim().takeIf { it.isNotBlank() })
                }.getOrNull()
            ...
            try {
                ...
                client.newCall(req).execute().use { resp ->
                    ...
                    parsed.metas.take(10).forEach { meta ->
                        ...
                        if (ttId != null) {
                            resolver.rememberTtId(numericId, ttId)
                        }
                        results.add(
                            WyzieTmdbResult(
                                id = numericId,
    ...
    suspend fun getTvShowDetails(id: Int): Result<WyzieTvShowDetails> = withContext(Dispatchers.IO) {
        try {
            ...
            val ttId = resolver.findTtIdForNumericId(id, "series")
            if (ttId != null) {
    ...
    private fun cinemetaGetTvShowDetails(ttId: String, originalId: Int): WyzieTvShowDetails {
        ...
        client.newCall(req).execute().use { resp ->
            ...
            val seasonMap = mutableMapOf<Int, MutableList<app.aryan447.mpvium.repository.subtitles.CinemetaVideo>>()
    ...
    suspend fun getSeasonEpisodes(id: Int, season: Int, imdbId: String? = null): Result<List<WyzieEpisode>> = withContext(Dispatchers.IO) {
        try {
            val targetTtId = imdbId ?: resolver.findTtIdForNumericId(id, "series")
            if (targetTtId != null) {
