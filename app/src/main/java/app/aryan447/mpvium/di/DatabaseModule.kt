...
val DatabaseModule =
  module {
    ...
    single<MpviumDatabase> {
      val context = androidContext()
      Room
        .databaseBuilder(context, MpviumDatabase::class.java, "mpvium.db")
    ...
    single<RecentlyPlayedRepository> {
      RecentlyPlayedRepositoryImpl(get<MpviumDatabase>().recentlyPlayedDao())
    }
    ...
    single {
      app.aryan447.mpvium.database.repository.VideoMetadataCacheRepository(
        ...
        dao = get<MpviumDatabase>().videoMetadataDao(),
      )
    }
    ...
    single {
      get<MpviumDatabase>().networkConnectionDao()
    }
    ...
    single {
      PlaylistRepository(
        playlistDao = get<MpviumDatabase>().playlistDao(),
      )
    }
  }
